...
def main():
    ...
    # POPUP LAYER
    popup_layer = folium.FeatureGroup(name='popup', show=False)
    ...
    for feature in temp_geojson.data['features']:
        ...
        folium.map.Popup(max_width=2650).add_child(folium.Vega(vis1, width=450, height=250)).add_to(temp_layer)
        # consolidate individual features back into the main layer
        temp_layer.add_to(popup_layer)

    # add main layer to folium.Map object
    popup_layer.add_to(map_graph)


    # saving map
    print('saving map')
    map_graph.save(project_dir + r'\data\final\unemployment_map_popups.html')
    map_graph.save(project_dir + r'\templates\unemployment_map_popups.html')
